...
const PQ: [&str; 40] = [
    "LP", "LL", "FC", "PF", "RF", "MQ", "MM", "NB", "CC", "CS", "MB", "FM", "GS", "DO", "DP", "CP",
    "NR", "CM", "CD", "MD", "HS", "VH", "CH", "MH", "CL", "XL", "CX", "HX", "RP", "TU", "TC", "FB",
    "FN", "PA", "PB", "SS", "MC", "SM", "GA", "GB",
];
//run through checks
pub fn verify(mut uen: String) -> (String, bool) {
    ...
    if uen.len() == 9 {
        ...
        if !all_num(uen.slice(..8)) {
            return (
                "non numerical characters found in UEN, or wrong number of characters".to_string(),
                false,
            );
        }
        ...
        if !all_num(uen.slice(5..9)) {
            return (
                "trailing characters contain invalid characters".to_string(),
                false,
            );
        }
